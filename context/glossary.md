...
- **Unmapped participant** — An in-scope participant whose GitHub login does not have a Slack user ID entry in `GITHUB_TO_SLACK_JSON`; they are skipped and reported.
- **`GITHUB_TO_SLACK_JSON`** — Repository or organization secret containing a JSON object that maps GitHub logins to Slack user IDs.
